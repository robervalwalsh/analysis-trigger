...
// L1 jet-mu objects
      ...
      std::vector<Jet> l1muonjetsbtv;
      std::vector<Muon> l1jetmuonsbtv;
// --------------------      
      // L1 jets selection
      ...
      if ( l1jet16.size() >= 1 && l1mu3.size() >= 1 )
      {
         for ( int j = 0 ; j < int(l1jet16.size()) ; ++j )
         {
            ...
            for ( int m = 0 ; m < int(l1mu3.size()) ; ++m )
            {
               ...
               if ( deltaR <= 0.4 )
               {
                  l1muonjetsbtv.push_back(jet);
                  l1jetmuonsbtv.push_back(muon);
               }
            }
         }
      }
...
// --------------------      
      ...
      trigger_accept_["L1_Mu3_Jet16"]        = ( l1mu3.size() >= 1 && l1muonjetsbtv.size() >= 1 && l1jet16.size() >= 1 && analysis.triggerResult("HLT_L1SingleMu3_v") ); // the last requirement is to ensure the L1 muon quality selection, not included in the ntuple
...
// HLT Muon-Jet objects
      ...
      std::vector<Jet>  l3pfmuonjets;
      std::vector<Muon> l3pfjetmuons;
// --------------------      
      ...
      if ( pfjets40.size() >= 1 && l3muon12.size() >= 1 )
      {
         for ( int j = 0; j < int(pfjets40.size()) ; ++j )
         {
            ...
            for ( int m = 0; m < int(l3muon12.size()); ++m )
            {
               ...
               if ( deltaR <= 0.4 ) 
               {   
                  l3pfmuonjets.push_back(jet);
                  l3pfjetmuons.push_back(muon);
               }
            }
         }
      }
...
//      hltPFMuJet = ( l2pfmuonjets.size() >= 1 && l3pfmuonjets.size() >= 1 );
      ...
      // Semileptonic
      trigger_accept_["HLT_2CaloJets30_Muon12_2CaloBTagCSV084_2PFJets40"] = ( calojets30.size() >= 2 && bjets30wp084.size() >= 2 && pfjets40.size() >= 2 && pfjets40deta.size() >= 2 && l3pfmuonjets.size() >= 1 && l2muon8.size() >= 1 && l3muon12.size() >= 1 );
      trigger_accept_["HLT_2CaloJets30_Muon12_2CaloBTagCSV092_2PFJets40"] = ( calojets30.size() >= 2 && bjets30wp092.size() >= 2 && pfjets40.size() >= 2 && pfjets40deta.size() >= 2 && l3pfmuonjets.size() >= 1 && l2muon8.size() >= 1 && l3muon12.size() >= 1 );
      // monitor
      trigger_accept_["HLT_2CaloJets30_Muon12_1CaloBTagCSV092_2PFJets40"]  = ( calojets30.size() >= 2 && bjets30wp092.size() >= 1 && pfjets40.size()  >= 2 && l3pfmuonjets.size() >= 1 && l2muon8.size() >= 1 && l3muon12.size() >= 1 );
      trigger_accept_["HLT_2CaloJets30_Muon12_1CaloBTagCSV092_2PFJets100"] = ( calojets30.size() >= 2 && bjets30wp092.size() >= 1 && pfjets100.size() >= 2 && l3pfmuonjets.size() >= 1 && l2muon8.size() >= 1 && l3muon12.size() >= 1 );
      trigger_accept_["HLT_2CaloJets30_Muon12_1CaloBTagCSV092_2PFJets200"] = ( calojets30.size() >= 2 && bjets30wp092.size() >= 1 && pfjets200.size() >= 2 && l3pfmuonjets.size() >= 1 && l2muon8.size() >= 1 && l3muon12.size() >= 1 );
      trigger_accept_["HLT_2CaloJets30_Muon12_1CaloBTagCSV092_2PFJets350"] = ( calojets30.size() >= 2 && bjets30wp092.size() >= 1 && pfjets350.size() >= 2 && l3pfmuonjets.size() >= 1 && l2muon8.size() >= 1 && l3muon12.size() >= 1 );
...
// --------------------      
      ...
      // All had monitor
      if ( trigger_accept_["L1_DoubjeJet100"] )
      {
         if ( trigger_accept_["HLT_2CaloJets100_1CaloBTagCSV092_2PFJets100"] )
         {
         ...
         if ( trigger_accept_["HLT_2CaloJets100_1CaloBTagCSV092_2PFJets140"] )
         {
         ...
         if ( trigger_accept_["HLT_2CaloJets100_1CaloBTagCSV092_2PFJets200"] )
         {
         ...
         if ( trigger_accept_["HLT_2CaloJets100_1CaloBTagCSV092_2PFJets350"] )
         {
      ...
      // semilep monitor
      if ( trigger_accept_["L1_Mu3_Jet16"] )
      {
         if ( trigger_accept_["HLT_2CaloJets30_Muon12_1CaloBTagCSV092_2PFJets40"] )
         {
         ...
         if ( trigger_accept_["HLT_2CaloJets30_Muon12_1CaloBTagCSV092_2PFJets100"] )
         {
         ...
         if ( trigger_accept_["HLT_2CaloJets30_Muon12_1CaloBTagCSV092_2PFJets200"] )
         {
         ...
         if ( trigger_accept_["HLT_2CaloJets30_Muon12_1CaloBTagCSV092_2PFJets350"] )
         {
