...
if ( $trigger == "HLT_PFJet100" ) then
   ...
   set l3ptmin = 100
   set l3etamax = 3.0
else if ( $trigger == "HLT_PFJet100er2p3" ) then
   set l1ptmin = 100
   set l1etamax = 2.3
   set l2ptmin = 100
   set l2etamax = 2.3
...
@ count = 1
while ( $count < $#ptmin + 1 )
   set er = `echo $l3etamax | sed -e "s/\./p/g"`
   set cfgbase = "pfjet_"$dataset"_"$era"_hlt"$l3ptmin"er"$er"_refhlt"$refpt[$count]"_pt"$ptmin[$count]"to"$ptmax[$count]
